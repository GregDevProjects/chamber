...
class RobotDialogue extends DialogueManager {
  ...
  drawDialogueBubble() {
    ...
    for (let i = 0; i < 360; i += this.step) {
      const x =
        this.radius * Math.cos(this.degreesToRadians(i)) + this.anchor.x;
      const y =
        this.radius * Math.sin(this.degreesToRadians(i)) + this.anchor.y;
      ...
      if (!this.movingPoints[i]) {
        ...
        const newx = this.movingPoints[i]
          ? this.movingPoints[i].x
          : (this.radius + newRad) *
              Math.cos(this.degreesToRadians(i + this.step / 2)) +
            this.anchor.x;
        const newy = this.movingPoints[i]
          ? this.movingPoints[i].y
          : (this.radius + newRad) *
